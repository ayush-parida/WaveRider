...
namespace EasyMobile
{
    public class MoPubClientImpl : AdClientImpl
    {
        ...
        /// </summary>
        public event Action<string> OnSdkInitalizedEvent
        {
            add { MoPubManager.OnSdkInitalizedEvent += value; }
            remove { MoPubManager.OnSdkInitalizedEvent -= value; }
        }
        ...
        protected MoPubBase.SdkConfiguration GetSdkConfiguration()
        {
            return new MoPubBase.SdkConfiguration
            {
                ...
                AdvancedBidders = GetAdvancedBidders(),
                MediationSettings = GetMediationSettings(),
                NetworksToInit = GetInitNetworks()
            };
        }

        protected MoPubBase.MediationSetting[] GetMediationSettings()
        {
            List<MoPubBase.MediationSetting> mediationSettings = new List<MoPubBase.MediationSetting>();
            foreach (var setting in mAdSettings.MediationSettings)
            {
                MoPubBase.MediationSetting newSetting = new MoPubBase.MediationSetting(setting.AdVendor);
                foreach (var option in setting.ExtraOptions)
                {
                    newSetting.Add(option.Key, option.Value);
                }
            }
            return mediationSettings.ToArray();
        }

        protected MoPubBase.AdvancedBidder[] GetAdvancedBidders()
        {
            var advancedBidders = new List<MoPubBase.AdvancedBidder>(mAdSettings.AdvancedBidders.Length);
            var existBidder = new List<MoPubSettings.MoPubAdvancedBidder>(mAdSettings.AdvancedBidders.Length);
            foreach (var bidder in mAdSettings.AdvancedBidders)
            {
                if (existBidder.Contains(bidder))
                {
                    Debug.LogWarning("Ignore duplicated bidder: " + bidder);
                    continue;
                }

                advancedBidders.Add(GetAdvanceBidderFromEnum(bidder));
                existBidder.Add(bidder);
            }
            return advancedBidders.ToArray();
        }

        protected MoPubBase.RewardedNetwork[] GetInitNetworks()
        {
            var initNetworks = new List<MoPubBase.RewardedNetwork>(mAdSettings.InitializeNetworks.Length);
            var existNetworks = new List<MoPubSettings.MoPubInitNetwork>(mAdSettings.InitializeNetworks.Length);
            foreach (var network in mAdSettings.InitializeNetworks)
            {
                if (existNetworks.Contains(network))
                {
                    Debug.LogWarning("Ignore duplicated network:" + network);
                    continue;
                }

                initNetworks.Add(GetInitNetworksFromEnum(network));
                existNetworks.Add(network);
            }
            return initNetworks.ToArray();
        }

        protected MoPubBase.AdvancedBidder GetAdvanceBidderFromEnum(MoPubSettings.MoPubAdvancedBidder bidder)
        {
            switch (bidder)
            {
                case MoPubSettings.MoPubAdvancedBidder.AdColony:
                    return MoPubBase.AdvancedBidder.AdColony;
                case MoPubSettings.MoPubAdvancedBidder.Facebook:
                    return MoPubBase.AdvancedBidder.Facebook;
                default:
                    return null;
            }
        }

        protected MoPubBase.RewardedNetwork GetInitNetworksFromEnum(MoPubSettings.MoPubInitNetwork network)
        {
            switch (network)
            {
                case MoPubSettings.MoPubInitNetwork.AdColony:
                    return MoPubBase.RewardedNetwork.AdColony;
                case MoPubSettings.MoPubInitNetwork.AdMob:
                    return MoPubBase.RewardedNetwork.AdMob;
                case MoPubSettings.MoPubInitNetwork.Chartboost:
                    return MoPubBase.RewardedNetwork.Chartboost;
                case MoPubSettings.MoPubInitNetwork.Facebook:
                    return MoPubBase.RewardedNetwork.Facebook;
                case MoPubSettings.MoPubInitNetwork.TapJoy:
                    return MoPubBase.RewardedNetwork.Tapjoy;
                case MoPubSettings.MoPubInitNetwork.UnityAds:
                    return MoPubBase.RewardedNetwork.Unity;
                case MoPubSettings.MoPubInitNetwork.Vungle:
                    return MoPubBase.RewardedNetwork.Vungle;
                default:
                    return null;
            }
        }
        ...
        {
            MoPubManager.OnSdkInitalizedEvent += OnSdkInitializedHandle;
        }
